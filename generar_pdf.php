...
// Si no tienes TCPDF, usar una solución simple con HTML
function generarPDFSimple($tipo, $conexion) {
    $html = '<!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
    ...
    switch($tipo) {
        case 'diario':
            $html .= generarReporteDiarioHTML($conexion);
            break;
        case 'mensual':
            $html .= generarReporteMensualHTML($conexion);
            break;
        case 'historial':
            $html .= generarHistorialCompletoHTML($conexion);
            break;
    }
...
function generarReporteDiarioHTML($conexion) {
...
function generarReporteMensualHTML($conexion) {
    $mes = date('m');
    $anio = date('Y');
...
function generarHistorialCompletoHTML($conexion) {
...
// Generar el PDF/HTML
generarPDFSimple($tipo_reporte, $conexion);
?>
